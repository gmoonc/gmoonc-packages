...
test.describe('Criação de Configuração de Notificação - Análises', () => {
  test('deve criar uma configuração de notificação para análises', async ({ page }) => {
    console.log('🎯 Testando criação de configuração de notificação para análises');
    ...
    // Selecionar categoria de análises
    console.log('🔍 Selecionando categoria de análises...');
    // O elemento é um combobox, não um select tradicional
    ...
    if (await categorySelect.isVisible({ timeout: 3000 })) {
      ...
      if (await categoryOption.isVisible({ timeout: 3000 })) {
        ...
        if (options.length > 1) {
          ...
          throw new Error('❌ Nenhuma categoria disponível no select. Certifique-se de que existe pelo menos uma categoria ativa de análises.');
        }
      }
    } else {
    ...
    console.log('🎯 Teste de criação de configuração de análises concluído com sucesso');
  });
});
