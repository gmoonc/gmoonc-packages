...
interface GMooncMensagemFormProps {
  initialData?: Mensagem | null;
  onSubmit: (formData: MensagemFormData) => void;
  onCancel: () => void;
}

export function GMooncMensagemForm({ initialData, onSubmit, onCancel }: GMooncMensagemFormProps) {
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.mensagem.trim()) {
      newErrors.mensagem = 'Message is required';
    }
        ...
        <div className="form-grid grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="form-group md:col-span-2">
            <label htmlFor="mensagem" className="form-label block text-sm font-medium text-gray-700 mb-2">
              Message <span className="required text-red-500">*</span>
            </label>
            <textarea
              id="mensagem"
              rows={6}
